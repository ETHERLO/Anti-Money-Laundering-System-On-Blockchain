...
const Transaction = () => {
  ...
  useEffect(()=>{
   ...
   const Info = async () => {
    ...
    var contractaddress = "0x9c77c4780012515731E042a4E10ec780c97776aF";
     ...
     const instance = new web3.eth.Contract(abi, contractaddress);
      ...
      var  investd =  await instance.methods.getAllCustomer().call();
       console.log(investd);
   //     setinvestId(investd[0]);
    ...
    for(var i = 0 ; i < investd.length  ; i++){
         ...
         var  totalRefBonu =  await instance.methods.customer_Detail(investd[i].eth_Adress).call();
         // var  order =  await instance.methods.orders(investd[i]).call();
         console.log("ye hai order " ,totalRefBonu);
        
           if(totalRefBonu.kyc == true){
 
          
            
           inv.push({ address : investd[i].eth_Adress , name : investd[i].first_Name  , nin : investd[i].national_Identity_Number, phone : investd[i].phone_Number, bank : investd[i].affilated_Bank}) ;
  ...
  return (
    ...
    <th>ETH Address</th>
  <th>Customer Name</th>
  <th>Customer National Identity Number</th>
  <th>Customer Phone Number</th>
  <th>Affilated Bank</th>
    </tr>
   </thead>
   <tbody>
   {investlen.length > 0 &&
