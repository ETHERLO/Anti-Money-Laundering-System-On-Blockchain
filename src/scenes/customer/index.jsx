...
var contractaddress = "0x9c77c4780012515731E042a4E10ec780c97776aF";
     ...
     await instance.methods.addCustomer(ethaddress , name , lname , dob , email , nin , phone , affbank , img , fimg , bimg).send({
          from : account
        });
 ...
 const handleFormSubmit = (values) => {
  ...
  const handleAffbank = (e) => {
    setAffbank(e.target.value);
    console.log(e.target.value)
  };
  ...
  const handleBimg = (e) => {
    setBimg(e.target.value);
    console.log(e.target.value)
  };


  const projectId = '2O5Wir4dxBijQYRqg0GIv0oSS4S';
  const projectSecret = 'dd81fb15ddd773a5df42fb5ccd9b9c02';
  ...
  const handleImageUploadF = async (file) => {
    try {
      const added = await ipfs.add(file);
      setFimg(added.cid.toString());
      console.log(added.cid.toString());
    } catch (error) {
      console.log(error);
    }
  ...
  const handleImageUploadB = async (file) => {
    try {
      const added = await ipfs.add(file);
      setBimg(added.cid.toString());
      console.log(added.cid.toString());
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <Box m="20px">
      <Header title="Add Customer Laundering Reporting Officer" subtitle="Add laundering reporting officer" />
...
<div class="form-group mt-3">
            <label class="mr-2">Upload Customer Image:</label>
            <input onChange={(event) => handleImageUpload(event.target.files[0])}  type="file" name="file"/>
          </div>

          <div class="form-group mt-3">
            <label class="mr-2">Upload Front ID Image:</label>
            <input  onChange={(event) => handleImageUploadF(event.target.files[0])} type="file" name="file"/>
          </div>

          <div class="form-group mt-3">
            <label class="mr-2">Upload Back ID Image:</label>
            <input onChange={(event) => handleImageUploadB(event.target.files[0])}  type="file" name="file"/>
          </div>
