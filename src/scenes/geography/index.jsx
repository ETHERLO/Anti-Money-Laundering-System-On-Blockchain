...
const Geography = () => {
  ...
  useEffect(()=>{
   ...
   const Info = async () => {
    ...
    var contractaddress = "0x9c77c4780012515731E042a4E10ec780c97776aF";
     ...
     const instance = new web3.eth.Contract(abi, contractaddress);
      ...
      var  investd =  await instance.methods.getAllTransaction().call();
       console.log(investd);
   //     setinvestId(investd[0]);
    ...
    for(var i = 0 ; i < investd.length  ; i++){
        ...
        //  var  totalRefBonu =  await instance.methods.org_Detail(investd[i].eth_Adress).call();
        //  // var  order =  await instance.methods.orders(investd[i]).call();
        // //  console.log("ye hai order " ,totalRefBonu);
        
           // if(order[7] == false){
 
          
            
           inv.push({ transaction_id : investd[i].transaction_id, Saddress : investd[i].sender_Adress , Sname : investd[i].sender_Name ,  Raddress : investd[i].receiver_Adress , Rname : investd[i].receiver_Name ,Amount : investd[i].amount , time :investd[i].timestamp }) ;
   ...
   const Add = async (add , name , radd , rname , amount ,id) => {
    window.web3 = new Web3(window.ethereum);
    await window.ethereum.enable();
    const accounts = await window.web3 .eth.getAccounts();
      const account = accounts[0];
   ...
   var contractaddress = "0x9c77c4780012515731E042a4E10ec780c97776aF";
    ...
    const instance = new web3.eth.Contract(abi, contractaddress);
     ...
     var  investd =  await instance.methods.addTransactionSus( add , name , radd , rname , amount ,id).send({from : account});
      console.log(investd);
  //     setinvestId(investd[0]);
  ...
  const columns = [
    { field: "id", headerName: "ETH Address", flex: 0.5 },
...
<tr key={index}>
  ...
  <td>{st.Raddress}</td>
  <td>{st.Rname}</td>
  <td>{st.Amount / 1000000000000000000 }</td>
  <td>{st.time}</td>
  <td><button style={{pointer :"curser" }}  onClick={()=> Add(st.Saddress , st.Sname , st.Raddress ,st.Rname ,st.Amount  , st.transaction_id  )} >Add Suspect </button> </td>
