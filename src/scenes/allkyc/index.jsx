...
var contractaddress = "0x9c77c4780012515731E042a4E10ec780c97776aF";
     ...
     await instance.methods.approveKyc(add).send({
          from : account
        });
   ...
   const Info = async () => {
    ...
    var contractaddress = "0x9c77c4780012515731E042a4E10ec780c97776aF";
     ...
     const instance = new web3.eth.Contract(abi, contractaddress);
      ...
      var  investd =  await instance.methods.getAllKyc().call();
       console.log(investd);
   //     setinvestId(investd[0]);
    ...
    for(var i = 0 ; i < investd.length  ; i++){
        ...
        //  var  totalRefBonu =  await instance.methods.org_Detail(investd[i].eth_Adress).call();
         var  order =  await instance.methods.kyc_req(investd[i].eth_Adress).call();
        // //  console.log("ye hai order " ,totalRefBonu);
           ...
           inv.push({ Saddress : investd[i].eth_Adress }) ;
...
<tr key={index}>
  <td>{index}</td>
  <td>{st.Saddress}</td>
  <td> <button style={{cursor :"pointer"}} onClick={()=> Accept(st.Saddress)} >  Accept </button> </td>
